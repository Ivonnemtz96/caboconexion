...
if(isset($_REQUEST['delId']) and $_REQUEST['delId']!=""){
    ...
    $Delfoto = $db->getAllRecords('fotosEmp','*','AND id='.($_REQUEST['delId']).' AND usuario='.($UserData['id']).'','LIMIT 1');
        ...
        $archivo = '../../upload/empresas/'.(strftime("%Y/%m", strtotime(($empSel['fr'])))).'/'.($Delfoto['codigo']).'.jpg';
        unlink($archivo); //BORRAMOS LA FOTO ANTIGUA SACANDO EL NOMBRE DE LA BASE DE DATOS

    
    setcookie("msg","fbok",time() + 2, "/");
	header('location: /panel/editar/galeria/?empId='.($empSel['id']).'');
	exit;
}
?>