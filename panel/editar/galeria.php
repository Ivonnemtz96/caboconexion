...
if(isset($_REQUEST['submit']) and $_REQUEST['submit']!=""){
	...
	if(($_FILES['thumb']['tmp_name'])==""){
		...
		header('location:/panel/editar/galeria/?empId='.($empresa['id']).'');
		exit;
	}else{
            ...
            if(($_FILES['thumb']['size']) > 1000000){ 
		      ...
		      exit;
                ...
                $ruta = '../../upload/empresas/'.(strftime("%Y/%m", strtotime(($empresa['fr'])))).'';
		...
		$FotosCount	=	$db->getQueryCount('fotosEmp','id');
		if($FotosCount[0]['total']<10000){
			...
			$insert	=	$db->insert('fotosEmp',$data);
			...
			if($insert){
				...
				header('location:/panel/editar/galeria/?empId='.($empresa['id']).'');//exito
				exit;
			}else{
                setcookie("msg","ups",time() + 2, "/");
				header('location:/panel/editar/galeria/?empId='.($empresa['id']).'');//sin cambios
				exit;
			}
		} else{
            setcookie("msg","lim",time() + 2, "/");
			header('location:/panel/editar/galeria/?empId='.($empresa['id']).''); //limite
			exit;
		      }
                        ...
                        <div class="container">
                            ...
                            <div class="profile-edit-wrap">
                                <div class="profile-edit-page-header">
                                    ...
                                    <div class="breadcrumbs"><a href="/">Inicio</a><a href="/panel/mis-empresas">Mis empresas</a><span>Galria</span></div>
                                </div>
                                <div class="row">
                                    <div class="col-md-3">
                                    ...
                                    ?>
                                    <?php $fotosData = $db->getAllRecords('fotosEmp','*','AND empresa='.($empresa['id']).'','ORDER BY id DESC LIMIT '.($empresa['fotCount']).'');
                                                ...
                                                ?>
                                                    ...
                                                    <div class="gallery-item">
                                                        <div class="grid-item-holder">
                                                            <div class="box-item">
                                                                <img  src="/upload/empresas/<?php echo (strftime("%Y", strtotime(($empresa['fr']))));?>/<?php echo (strftime("%m", strtotime(($empresa['fr']))));?>/<?php echo ($foto['codigo']) ?>.jpg"   alt="">
                                                                <a href="/upload/empresas/<?php echo (strftime("%Y", strtotime(($empresa['fr']))));?>/<?php echo (strftime("%m", strtotime(($empresa['fr']))));?>/<?php echo ($foto['codigo']) ?>.jpg" class="gal-link popup-image"><i class="fa fa-search-plus"  ></i></a>
                                                            </div>
                                                            <a style="color: red; margin-top: 10px;" href="/panel/borrar/foto?delId=<?php echo ($foto['id']); ?>"><i class="fa fa-times" ></i>Borrar</a>
                                                        </div>
                                                    </div>
                                                <?php  
                                            }
