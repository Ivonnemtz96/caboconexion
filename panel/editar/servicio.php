...
if (isset($_REQUEST['editId']) and $_REQUEST['editId']!=""){
	$serSel  =  $db->getAllRecords('servicios','*','AND id='.($_REQUEST['editId']).' AND usuario='.($UserData['id']).'','LIMIT 1');
}

if (empty($serSel)) { //SI NO EXISTE ES QUE NO HAY UN ID DE PROPIEDAD VÁLIDO Y REDIRECCIONAMOS Y LANZAMOS ERROR
        setcookie("msg","ups",time() + 2, "/");
        header('location:/panel/mis-empresas');
        exit;
    }

$serSel  =  $serSel [0]; //PASAMOS LOS PRIMEROS 2 FILTROS Y SI TENEMOS UNA PROPIEDAD VÁLIDA SELECCIONADA 
...
if(isset($_REQUEST['submit']) and $_REQUEST['submit']!=""){
	...
	if(($nombre=="")&($categoria=="")&($telefono=="")&($email=="")&($descripcion=="")){
		...
		header('location:/panel/editar/servicio?&editId='.$_REQUEST['editId'].'');
		exit;
	}else if($nombre==""){
        ...
        $codigo = ($serSel['fPortada']); //SI NO SE SUBE LA FOTO LE DAMOS EL VALOR EXISTENTE QUE YA ESTÁ EN NUESTRA BASE DE DATOS
         
        if (!empty($_FILES['thumb']['tmp_name'])) {
            
            //A ESTE PUNTO SABEMOS QUE SI SUBIÓ UNA FOTO NUEVA, ENTONCES DEBEMOS BORRAR LA EXISTENTE
            if (isset($serSel['fPortada'])){
                $archivo = '../../upload/servicios/'.(strftime("%Y/%m", strtotime(($serSel['fr'])))).'/'.($serSel['fPortada']).'.jpg';
            ...
            if(($_FILES['thumb']['size']) > 1000000){ 
                ...
                $ruta = '../../upload/servicios/'.(strftime("%Y/%m", strtotime(($serSel['fr'])))).'';
        ...
        if(isset($_REQUEST['submit']) and $_REQUEST['submit']!=""){
			...
			$data	=	array(
                            ...
                            'catSubServicio'=>$subcategoria,
                            'lugar'=>$lugar,
                            'usuario'=>($UserData['id']),
                            
						);
            $update	=	$db->update('servicios',$data,array('id'=>($_REQUEST['editId'])));
        
            
			if($update){
                setcookie("msg","empedit",time() + 2, "/");
                header('location:/panel/editar/servicio?editId='.$_REQUEST['editId'].''); //Exito en el cmabio
                exit;}
            else{
                setcookie("msg","ups",time() + 2, "/");
                header('location:/panel/editar/servicio?editId='.$_REQUEST['editId'].''); //sin cambios
                exit;
            }
        }
    }
        ...
        $Lugar = $db->getAllRecords('lugar','*', 'AND id="'.($serSel['lugar']).'"LIMIT 1 ');
        $Lugar = $Lugar[0];
        
        //OBTENER EL CATEGORIA POR ID
        $Categoria = $db->getAllRecords('catServicios','*', 'AND id="'.($serSel['catServicio']).'"LIMIT 1 ');
        $Categoria = $Categoria[0];

        //OBTENER EL CATEGORIA POR ID
        $CategoriaSub = $db->getAllRecords('catSubServicios','*', 'AND id="'.($serSel['catSubServicio']).'"LIMIT 1 ');
                        ...
                        <div class="container">
                            ...
                            <div class="profile-edit-wrap">
                                <div class="profile-edit-page-header">
                                    <h2><i class="fa fa-plus-circle"></i> Estás editando: <?php echo ($serSel['nombre'])?></h2>
                                    <div class="breadcrumbs"><a href="/">Inicio</a><a href="/panel">Panel</a><span>Editar servicio</span></div>
                                </div>
                                        ...
                                        <!-- profile-edit-container--> 
                                                ...
                                                <div class="row">
                                                    <div class="col-md-6">
                                                        ...
                                                        <input name="nombre" type="text" value="<?php echo ($serSel['nombre'])?>" />
                                                    </div>
                                                    <div class="col-md-6">
                                                            <label>Lugar</label>
                                                        ...
                                                        <div class="col-md-6">
                                                            ...
                                                            <select name="categoria" data-placeholder="Seleccionar..." class="chosen-select" >
                                                               <option selected value="<?php echo ($Categoria['id']);?>">Actual: <?php echo ($Categoria['nombre']);?></option>
                                                               
                                                                <?php
                                                            $OptionData = $db->getAllRecords('catServicios','*','ORDER BY nombre ASC');
                                                            if (count($OptionData)>0){
                                                        ...
                                                        <div class="col-md-6">
                                                            ...
                                                            <select name="subcategoria" data-placeholder="Seleccionar..." class="chosen-select" >
                                                               <option selected value="<?php echo ($CategoriaSub['id']);?>">Actual: <?php echo ($CategoriaSub['nombre']);?></option>
                                                                ...
                                                                <?php
                                                            $OptionData = $db->getAllRecords('catSubServicios','*','ORDER BY nombre ASC');
                                                            if (count($OptionData)>0){
                                                                foreach($OptionData as $empresa){?>			    		
                                                                        <option value="<?php echo ($empresa['id']);?>"><?php echo ($empresa['nombre']);?></option>
                                                                        <?php 
                                                        ...
                                                        <div class="col-md-3">
                                                            <div style="margin-top: 20px;">
                                                                <div class="add-list-media-wrap">
                                                                    <img class="responsive" src="/upload/servicios/<?php echo (strftime("%Y/%m", strtotime(($serSel['fr']))));?>/<?php echo ($serSel['fPortada']) ?>.jpg" alt="">
                                                                </div>
                                                            </div>
                                                        </div>
                                                        ...
                                                        <div style="margin-top: 20px;" class="col-md-12">
                                                            ...
                                                            <textarea name="descripcion" cols="40" rows="3" placeholder=""><?php echo ($serSel['descripcion'])?></textarea>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                                ...
                                                <div class="custom-form">
                                                    <div class="row">
                                                        <div class="col-md-6">
                                                            ...
                                                            <input name="telefono" type="text" value="<?php echo ($serSel['telefono'])?>" />
                                                        </div>
                                                        <div class="col-md-6">
                                                            <label>Correo electrónico <i class="fa fa-envelope"></i></label>
                                                            <input name="email" type="text" value="<?php echo ($serSel['email'])?>" />
                                                        </div>
