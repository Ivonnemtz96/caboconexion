...
//OBTENER RANGO POR ID
    ...
    if (isset($_REQUEST['prodId']) and $_REQUEST['prodId']!=""){
        $empresa  =  $db->getAllRecords('empresas','*',' AND id="'.$_REQUEST['prodId'].'"LIMIT 1');
    }
    
    if (empty($empresa)) { //SI NO EXISTE ES QUE NO HAY UN ID VÁLIDO Y REDIRECCIONAMOS Y LANZAMOS ERROR
...
if(isset($_REQUEST['submit']) and $_REQUEST['submit']!=""){
	...
	if(($_FILES['thumb']['tmp_name'])==""){
		...
		header('location:?prodId='.($empresa['id']).'');
		exit;
	}else{
            ...
            if(($_FILES['thumb']['size']) > 1000000){ 
                ...
                $ruta = '../../upload/empresas/'.(strftime("%Y", strtotime(($empresa['fr'])))).'/'.(strftime("%m", strtotime(($empresa['fr'])))).'';
		...
		if($FotosCount[0]['total']<10000){
			...
			if($insert){
				...
				header('location:/admin/editar/galeria/?prodId='.($empresa['id']).'');//exito
				exit;
			}else{
                setcookie("msg","ups",time() + 2, "/");
				header('location:/admin/editar/galeria/?prodId='.($empresa['id']).'');//sin cambios
				exit;
			}
		} else{
            setcookie("msg","lim",time() + 2, "/");
			header('location:/admin/editar/galeria/?prodId='.($empresa['id']).''); //limite
			exit;
		}
	}
}
...
<head>
  ...
  <title>CaboConexion - Galería - Galería</title>
  <!-- General CSS Files -->
        ...
        <div class="section-body">
            <div class="row clearfix">
                <div class="col-md-12 col-lg-12 col-xl-8">
                    <div class="card">
                        <div class="card-header">
                            <h4>Galería de: <?php echo ($empresa['nombre']) ?> (<?php echo ($empresa['fotCount']) ?>)</h4>
                        </div>
                        <div class="card-body">
                                            ...
                                            foreach($fotosData as $foto){
                                                ...
                                                ?>
									            	<div class="col-lg-3 col-md-4 col-sm-6 col-xs-12">
                                                        ...
                                                        <img class="responsive" src="/upload/empresas/<?php echo (strftime("%Y", strtotime(($empresa['fr']))));?>/<?php echo (strftime("%m", strtotime(($empresa['fr']))));?>/<?php echo ($foto['codigo']) ?>.jpg" alt="">
