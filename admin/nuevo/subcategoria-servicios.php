...
if(isset($_REQUEST['submit']) and $_REQUEST['submit']!=""){
	...
	if($nombre==""){
		...
		header('location:/admin/nuevo/subcategoria-servicios');
		exit;
	}else{
        
		$caractCount	=	$db->getQueryCount('catSubServicios','id');
		if($caractCount[0]['total']<10000){
			...
			$insert	=	$db->insert('catSubServicios',$data);
			if($insert){
                setcookie("msg","subcatok",time() + 2, "/");
				header('location:/admin/nuevo/subcategoria-servicios');//exito
				exit;
			}else{
                setcookie("msg","ups",time() + 2, "/");
				header('location:/admin/nuevo/subcategoria-servicios');//sin cambios
				exit;
			}
		} else{
            setcookie("msg","lim",time() + 2, "/");
			header('location:/admin/nuevo/subcategoria-servicios'); //limite
			exit;
		}
	}
}
...
<head>
  ...
  <title>CaboConexion - Nueva - Categoría</title>
   <!-- General CSS Files -->
      ...
      <div class="main-content">
            ...
            <div class="col-md-12 col-lg-12 col-xl-8">
                ...
                <div class="card">
                    <div class="card-header">
                        <h4>Agregar una categoría nueva</h4>
                    </div>
                    <div class="card-body">
                        <div class="row">
                            <div class="col-sm-6">
                            ...
                            <div class="col-sm-6">
                                <div class="form-group">
                                    ...
                                    <select name="categoria" class="form-control selectric" required>
                                        ...
                                        <?php
                                        $catData = $db->getAllRecords('catServicios','*',' ORDER BY nombre ASC');
                                        if (count($catData)>0){
                                            $y	=	'';
        ...
        <div class="row">
            <div class="col-md-12 col-lg-12 col-xl-8">
                <div class="card">
                    <div class="card-header">
                      <h4>Todas las categorías actuales</h4>
                    </div>
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-striped" id="table-1">
                                ...
                                <tbody>
                                <?php $categoriaData = $db->getAllRecords('catSubServicios','*','ORDER BY id DESC');
                                  if (count($categoriaData)>0){
                                      $y	=	'';
                                          foreach($categoriaData as $categoria){
                                              
                                              $catPadre = $db->getAllRecords('catServicios','*',' AND id="'.($categoria['categoria']).'"LIMIT 1 ');
                                              $catPadre = $catPadre[0];
                                              ...
                                              <tr>
                                                  ...
                                                  <td><?php echo $catPadre['nombre']; ?></td>
                                                  <td class="text-rigth">
                                                      <a href="/admin/editar/subcategoria-servicios?editId=<?php echo $categoria['id']; ?>" class="btn btn-icon btn-primary"><i class="far fa-edit"></i></a>
                                                      <a href="/admin/borrar/subcategoria-servicios?delId=<?php echo $categoria['id']; ?>" onClick="return confirm('Estás seguro? Esto no se puede deshacer');" class="btn btn-icon btn-danger"><i class="fa fa-trash"></i></a>
                                                  </td>
                                              </tr>
