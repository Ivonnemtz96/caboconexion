...
if(isset($_REQUEST['submit']) and $_REQUEST['submit']!=""){
	...
	if($nombre==""){
		...
		header('location:/admin/nuevo/subcategoria-empresas');
		exit;
	}else{
        
		$caractCount	=	$db->getQueryCount('catSubEmpresa','id');
		if($caractCount[0]['total']<10000){
			...
			$insert	=	$db->insert('catSubEmpresa',$data);
			if($insert){
                setcookie("msg","catok",time() + 2, "/");
				header('location:/admin/nuevo/subcategoria-empresas');//exito
				exit;
			}else{
                setcookie("msg","ups",time() + 2, "/");
				header('location:/admin/nuevo/subcategoria-empresas');//sin cambios
				exit;
			}
		} else{
            setcookie("msg","lim",time() + 2, "/");
			header('location:/admin/nuevo/subcategoria-empresas'); //limite
			exit;
		}
	}
}
                            ...
                            <div class="col-sm-6">
                                <div class="form-group">
                                    ...
                                    <select name="categoria" class="form-control selectric" required>
                                        ...
                                        <?php
                                        $catData = $db->getAllRecords('catEmpresa','*',' ORDER BY nombre ASC');
                                        if (count($catData)>0){
                                            $y	=	'';
                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-striped" id="table-1">
                                ...
                                <tbody>
                                <?php $categoriaData = $db->getAllRecords('catSubEmpresa','*','ORDER BY id DESC');
                                  if (count($categoriaData)>0){
                                      $y	=	'';
                                          foreach($categoriaData as $categoria){
                                              
                                              $catPadre = $db->getAllRecords('catEmpresa','*',' AND id="'.($categoria['categoria']).'"LIMIT 1 ');
                                              $catPadre = $catPadre[0];
                                              ...
                                              <tr>
                                                  ...
                                                  <td class="text-rigth">
                                                      <a href="/admin/editar/subcategoria-empresas?editId=<?php echo $categoria['id']; ?>" class="btn btn-icon btn-primary"><i class="far fa-edit"></i></a>
                                                      <a href="/admin/borrar/subcategoria-empresas?delId=<?php echo $categoria['id']; ?>" onClick="return confirm('Estás seguro? Esto no se puede deshacer');" class="btn btn-icon btn-danger"><i class="fa fa-trash"></i></a>
                                                  </td>
                                              </tr>
