...
if(isset($_REQUEST['submit']) and $_REQUEST['submit']!=""){
	...
	if($nombre==""){
		...
		header('location:/admin/nuevo/subcategoria-productos');
		exit;
	}else{
        
		$caractCount	=	$db->getQueryCount('catSubProd','id');
		if($caractCount[0]['total']<10000){
			...
			$insert	=	$db->insert('catSubProd',$data);
			if($insert){
                setcookie("msg","subcatok",time() + 2, "/");
				header('location:/admin/nuevo/subcategoria-productos');//exito
				exit;
			}else{
                setcookie("msg","ups",time() + 2, "/");
				header('location:/admin/nuevo/subcategoria-productos');//sin cambios
				exit;
			}
		} else{
            setcookie("msg","lim",time() + 2, "/");
			header('location:/admin/nuevo/subcategoria-productos'); //limite
			exit;
		}
	}
}
                            ...
                            <div class="col-sm-6">
                                <div class="form-group">
                                    ...
                                    <select name="categoria" class="form-control selectric" required>
                                        ...
                                        <?php
                                        $catData = $db->getAllRecords('catProducto','*',' ORDER BY nombre ASC');
                                        if (count($catData)>0){
                                            $y	=	'';
                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-striped" id="table-1">
                                ...
                                <tbody>
                                <?php $categoriaData = $db->getAllRecords('catSubProd','*','ORDER BY id DESC');
                                  if (count($categoriaData)>0){
                                      $y	=	'';
                                          foreach($categoriaData as $categoria){
                                              
                                              $catPadre = $db->getAllRecords('catProducto','*',' AND id="'.($categoria['categoria']).'"LIMIT 1 ');
                                              $catPadre = $catPadre[0];
                                              ...
                                              <tr>
                                                  ...
                                                  <td class="text-rigth">
                                                      <a href="/admin/editar/subcategoria-productos?editId=<?php echo $categoria['id']; ?>" class="btn btn-icon btn-primary"><i class="far fa-edit"></i></a>
                                                      <a href="/admin/borrar/subcategoria-productos?delId=<?php echo $categoria['id']; ?>" onClick="return confirm('Estás seguro? Esto no se puede deshacer');" class="btn btn-icon btn-danger"><i class="fa fa-trash"></i></a>
                                                  </td>
                                              </tr>
